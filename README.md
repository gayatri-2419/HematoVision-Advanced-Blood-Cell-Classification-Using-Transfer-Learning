...
Using a pretrained Convolutional Neural Network (**MobileNetV2**), the model achieves high accuracy while reducing training time and computational cost.
...
Instead of training from scratch, we fine-tune **MobileNetV2** to classify blood cell images efficiently.
...
### 5️⃣ Deployment
The trained model (**Blood_Cell.h5**) is deployed using the **Flask Web Framework**.
...
4. Resize to 224×224  
5. Apply `MobileNetV2 preprocess_input()`  
6. Model prediction  
7. `argmax()` selects predicted class  
8. Image encoded to Base64  
9. Result displayed in `result.html`
